...
const fallbackGradients = [
 { gradient: " ", glow: "shadow-orange-500/20" },
 { gradient: " ", glow: "shadow-amber-500/20" },
 { gradient: " ", glow: "shadow-blue-500/20" },
 { gradient: " ", glow: "shadow-green-500/20" },
];
...
export default function ServicesPage() {
 ...
 tag: "What We Offer",
 title: "Our Services",
 subtitle: "From intimate dinner parties to grand celebrations, we bring culinary excellence and professional service to every occasion."
 });
 ...
 if (parsed.servicesPage) {
 setBanner({
 tag: parsed.servicesPage.bannerTag || "What We Offer",
 title: parsed.servicesPage.bannerTitle || "Our Services",
 subtitle: parsed.servicesPage.bannerSubtitle || "From intimate dinner parties to grand celebrations, we bring culinary excellence and professional service to every occasion."
 });
 }
 if (parsed.about) {
 const loadedStats = [
 ...
 <Link href="/contact"
 className="bg-stone-950 hover:bg-stone-900 text-white dark:bg-white dark:hover:bg-stone-100 dark:text-stone-950 border-0 px-7 py-3 font-bold text-sm rounded-full shadow-xl hover:scale-[1.02] transition-all duration-200 flex items-center justify-center gap-2">
 ...
 className="text-center p-4 glass-card rounded-2xl border border-orange-100/30 dark:border-orange-900/20 hover:scale-[1.01] transition-transform duration-300"
 >
 ...
 className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-full transition-all duration-200 border ${
 activeCategory === cat
 ? "bg-[#c05c31] text-white border-[#c05c31] shadow-lg shadow-orange-500/10"
 : "border-border text-muted-foreground hover:bg-orange-500/5 hover:border-orange-200"
 }`}
 >
 {cat}
 </button>
 ...
 {services.filter((s: any) => activeCategory === "All" || s.category === activeCategory).map((service: any, index: number) => {
 ...
 {service.badge && (
 <span className={`px-3 py-1 rounded-full text-xs font-bold ${gradient} text-white flex items-center gap-1.5`}>
 ...
 <div className={`w-5 h-5 rounded-full ${gradient} flex items-center justify-center flex-shrink-0 shadow-sm`}>
 ...
 <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${gradient} text-white text-sm font-bold shadow-lg`}>
 ...
 {/* Decorative blob */}
 <div className={`absolute -z-10 -bottom-6 ${index % 2 === 0 ? '-right-6' : '-left-6'} w-40 h-40 ${gradient} opacity-15 rounded-full blur-2xl`} />
 </div>
 </motion.div>
 );
 })}
 ...
 <div className="flex flex-col sm:flex-row gap-3 justify-center">
 <Link href="/contact"
 className="flex items-center justify-center gap-2 px-8 py-3.5 rounded-full font-bold text-base bg-white text-orange-600 hover:bg-orange-50 transition-all duration-200 shadow-2xl shadow-black/20 hover:scale-[1.02]">
