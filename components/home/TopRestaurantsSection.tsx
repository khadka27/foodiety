...
export function TopRestaurantsSection() {
 ...
 const onlyRestaurants = res.data.filter((e: any) => {
 const lowerCuisine = (e.cuisine || []).map((c: string) => c.toLowerCase());
 const lowerFeatures = (e.features || []).map((f: string) => f.toLowerCase());
 const isCafe = lowerCuisine.includes("cafe") || lowerFeatures.includes("cafe") || e.category === "FAST_FOOD";
 const isHotel = lowerCuisine.includes("hotel") || lowerFeatures.includes("hotel");
 return !isCafe && !isHotel;
 });
 ...
 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[500px] bg-[#c05c31]/5 rounded-full blur-[100px] -z-10 pointer-events-none" />
 ...
 <div className="h-px w-10 bg-[#c05c31]" />
